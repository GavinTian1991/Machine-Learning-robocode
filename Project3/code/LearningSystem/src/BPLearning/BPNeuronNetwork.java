...
public class BPNeuronNetwork {
	...
	public double errorCalculation(double arg, double actionStates[]) {
	...
	public  double[] selectTableInput(int state, int action) {
		...
		double actionTmp = ((double)action) / 5;
		input[numInput - 1] = actionTmp;
		return input;
	}
	    ...
	    try {
	    	...
	    	for (int i = 0; i < numLUTStates; i++)
	    		for (int j = 0; j < numLUTActions; j++) {
	    			double tmp = Double.parseDouble(read.readLine());
	    			if(tmp > max)
	    				max = tmp;
	    			table[i][j] = tmp;
	    		}
